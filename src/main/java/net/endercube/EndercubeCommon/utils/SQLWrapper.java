...
public class SQLWrapper {
     ...
     */
    public SQLWrapper(HikariDataSource dataSource, String tableName) {
    ...
    public void addTime(Player player, String course, Long time) {
        try {
            ...
            String sql = "INSERT INTO ?(player,course,time) VALUES(?,?,?)";

            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1, tableName);
            preparedStatement.setString(2, String.valueOf(player.getUuid()));
    ...
    public Long getTimePlayer(Player player, String course, int index) {
        try {
            Connection connection = dataSource.getConnection();
            String sql = "SELECT * FROM ? WHERE player = ? AND course = ? ORDER BY time ASC LIMIT 1 OFFSET ?;";

            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1, tableName);
            preparedStatement.setString(2, String.valueOf(player.getUuid()));
            preparedStatement.setString(3, course);
            preparedStatement.setInt(4, index - 1);
    ...
    public Long getTimeOverall(String course, int index) {
        try {
            Connection connection = dataSource.getConnection();
            String sql = "SELECT * FROM ? WHERE course = ? ORDER BY time ASC LIMIT 1 OFFSET ?;";
